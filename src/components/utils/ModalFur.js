...
class ModalFur extends Component {
    ...
    handleFurClick(event) {
        ...
        if (this.state.verifyFur) {
            if(this.props.pagina === 'PRESCRIPCION'){
                _fetch.fetchGet( 'api/Cobros/getFurPrescripcion/' + this.state.inputFur).then(fur =>{
                    //if (fur !== undefined && fur.status === true && fur.Fur !== null) {  //is ok
                        ...
                        input_fur.value = '102218';
                        document.cookie = "fur="+input_fur.value+"; expires="+today.toString();
                        document.cookie = "contribuyente=NATURAL; expires="+today.toString();

                        //modificar con moment
                        this.props.toast.success('OK', {
                            position: "top-right",
                            autoClose: 5000,
